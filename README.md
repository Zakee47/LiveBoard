...
VoiceBoard is a local-first React + Vite + TypeScript scaffold for a voice-controlled tldraw canvas. It starts from the tldraw Agent Starter Kit and adds stable contracts for the Action Bridge, Realtime Voice, Context, UI, and QA workstreams.

## VoiceBoard MVP scope

- Full-screen tldraw canvas.
- Right sidebar with voice state, activity, transcript, mock action runner, and the starter-kit chat panel.
- Bottom push-to-talk voice button.
- Stable contracts under `src/voice/`.
- Tiny Express token proxy at `localhost:3001`.
- No auth, persistence, or collaboration.
- Target browser: Chrome on macOS.
...
Run the token proxy in a second terminal:

```bash
npm run dev:proxy
```

The app currently uses mocked realtime behavior, so `OPENAI_API_KEY` is optional.

## VoiceBoard environment

Create `.env` for the proxy when you are ready to test the real OpenAI Realtime API:
...
`POST http://localhost:3001/api/realtime-token` returns a mock client secret when `OPENAI_API_KEY` is missing and requests a real ephemeral realtime session when it is present.

## VoiceBoard scripts

```bash
npm run dev        # Vite app at localhost:5173
npm run dev:proxy  # Express token proxy at localhost:3001
npm run build      # Production build
npm run typecheck  # TypeScript check
```

## VoiceBoard integration points

- `src/voice/types.ts` defines `VoiceState`, transcript entries, canvas snapshots, and tool/action contracts.
- `src/voice/tools.ts` exports OpenAI Realtime-compatible tool definitions for `create_shapes`, `update_shapes`, `delete_shapes`, `layout_shapes`, and `critique_canvas`.
- `src/voice/ActionBridge.ts` applies tool actions to a tldraw `Editor`.
- `src/voice/CanvasContextProvider.ts` snapshots shapes, selection, viewport, and bounds.
- `src/voice/VoiceSessionManager.ts` is a mocked session manager ready to be swapped for WebRTC/OpenAI Realtime.
- `src/voice/PushToTalkController.ts` exposes a minimal start/stop/toggle controller.
- `server/proxy.ts` is the token proxy for future WebRTC session setup.

Keep the exported contracts in `src/voice/` backward-compatible unless coordinating across workstreams. The current UI intentionally calls mocked behavior so branch work can proceed without a real `OPENAI_API_KEY`.
